(function (S, SL) {
    ...
    SL.IncidentsService = [
        ...
        function ($q, $cacheFactory, $log, utils, queueManager, incidentDataService, simplyLogApiClient, entityAttachmentsDataService) {
            ...
            function getIncidentDetails(id) {
                return $q.all([
                    incidentDataService.getIncident(id).then(createMapIncident()),
                    entityAttachmentsDataService.getAttachments("Event", id)
                ]).then(function (results) {

                    results[0].Attachments = results[1];
                    return results[0];
                });
            }
            ...
            function sendActions(incidentActions) {
                console.log("Sending incident actions... ", _.clone(incidentActions));
            ...
            function sendChecklistItems(checklistItems) {
                ...
                console.log("Sending incident actions... ", checklistItems);
            ...
            function sendIncident(incident) {
                console.log("Sending incident... ", _.extend({}, incident));
                if (incident == null) {
                    var d = $q.defer();
            ...
            function saveIncidentActions(incidentActions) {
                return $q.when(incidentActions).then(actionsQueue.push)
                    .then(function () {
            ...
            function saveChecklistItems(items) {
                return $q.when(items).then(checklistItemsQueue.push)
                    .then(function () {
            ...
            function deleteIncident(incidentId) {
                return incidentDataService.deleteIncident(incidentId).then(clearIncidentsCache);
            }

            function delegateTreatment(incidentId, info) {
                return simplyLogApiClient.delegateTreatment(incidentId, info).then(function () {
                    return deleteIncident(incidentId);
                });
            }

            function openIncident(id) {
                return simplyLogApiClient.openIncident(id).then(function () {
                    incidentDataService.updateIncidentStatus(id, SL.IncidentStatus.Open);
                });
            }

            function closeIncident(id) {
                return simplyLogApiClient.closeIncident(id).then(function () {
                    incidentDataService.updateIncidentStatus(id, SL.IncidentStatus.Closed);
                });
            }

            function updateStoredAttachments(attachments) {
                return $q.when(attachments);
                //return $q.all([
                //    entityAttachmentsDataService.addAttachments(_.filter(attachments, function(att) { return !att.Id; }))
                //]);
            }

            function deleteStoredAttachments(deletedAttachmentsIds) {
                return entityAttachmentsDataService.deleteAttachments(deletedAttachmentsIds);
            }

            function updateAttachments(id, attachments, deletedAttachments) {
                return simplyLogApiClient.updateAttachments(id, attachments, deletedAttachments)
                                .then(function () {
                                    return $q.all([
                                        deleteStoredAttachments(deletedAttachments),
                                        updateStoredAttachments(attachments)
                                    ]);
                                });
            }

            function queryCanCloseIncident(id) {
                return simplyLogApiClient.queryCanCloseIncident(id);
            }
