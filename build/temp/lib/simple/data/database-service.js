    ...
    S.DatabaseServiceProvider = function () {
        ...
        var get = ["$q", function getService($q) {
            ...
            function buildWhereExpression(tableName, expressions) {
                var clause = [],groupVisited = false, visited = false;
                _.each(expressions, function (group) {
                    if (groupVisited) {
                        clause.push(" AND ");
                    }
                    clause.push("(");
                    _.each(group, function (expression) {

                        if (visited) {
                            clause.push(expression[3] || " AND ");
                        }
                        clause.push("[");
            ...
            function addQueryArguments(args, expressions) {
                _.each(expressions, function (expression) {
                    if (_.isArray(expression[0])) {
                        _.each(expression, function(subExpression) {
                            args.push(subExpression[2]);
                        });
            ...
            function createTable(context, table, model) {
                ...
                var queryable = {
                    ...
                    where: function () {
                        ...
                        this.whereExpressions = this.whereExpressions || [];
                        this.whereExpressions.push(expressions);
                        return this;
                    },
                    orderBy: function () {
                        var expressions = _.map(arguments, function (item) { return item; });
                    ...
                    select: function () {
                        ...
                        if (!this.custom && fields.length > 0 && fields[0] != "*") {
                            fieldsExpression = _.map(fields, function (field) {
                                return field;
                            }).join(",");
                        } else {
                            //fieldsExpression = buildTableColumnsExpression(table, "t");
                            fieldsExpression = this.custom || "*";
                        }

                        var query = "SELECT " + (this.distinctQuery ? " DISTINCT " : "") + fieldsExpression + " FROM [" + this.table + "] t",
                            queryArgs = [];
                        ...
                        if (shouldLog()) {
                            console.log("SQL", query, queryArgs);
                        }
            ...
            return function (name, userName) {
                var dataBase = getDatabase(name, userName);
        ...
        function getDatabase(name, userName) {
            var fullName = name + (userName ? "-" + userName : "");
            var dbInfo = dataBaseModels[name];
            var builder = new DatabaseBuilder(dbInfo.model);
            var dataBase = dataBases[fullName] || {
                model: dbInfo.model,
                name: fullName,
                dataBase: verifyDatabase(fullName, dbInfo.model, builder.build)
            };

            dataBases[fullName] = dataBase;
            return dataBases[fullName];
        }

        function configure(model) {
            dataBaseModels[model.Name] = {
                name: model.Name,
                model: model
            };
        }

        function verifyDatabase(fullName, model, createCallback, errorCallback) {
            try {
