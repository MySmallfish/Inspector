(function (S) {
    ...
    S.QueueManager = ["$q", "storageService", "$log", function ($q, storageService, $log) {
        var queues = {};
            ...
            function pushAction(item) {
                return function() {
                    return storageService.local(queueName).then(function(queueItems) {
                        $log.info("Queued Items:", queueItems.length);
                        ...
                        $log.info("Storing Queued Items:", queueItems.length);
                ...
                if (!inProcess) {
                    ...
                    return storageService.local(pendingQueueName).then(function (items) {
                        ...
                        return $q.all(_.map(items, function (item) {
                            $log.info("Processing Item:", item.length);
